import { PatternCrafter } from "./patterncrafter.js";
import { DefaultSpecification } from "../template-spec/template_specification.js";
import { applyMixins } from '../utils/mixin.js';
...
class PatternBuilderBase {
    ...
    build(spec) {
        ...
        if (crafted.length === 0)
            throw new Error('Something went wrong crafting the pattern');
        return crafted[0];
    }
    ...
    pickSpecification(data, spec) {
        return (spec) ? new spec(data) : new DefaultSpecification(data);
    }
    addTemplateVariable(target, data, input) {
...
class PatternFilterBuilder extends PatternBuilderBase {
    ...
    filter(exceptions) {
        var _a;
        const symbol = (_a = this.pattern.settings.symbol) !== null && _a !== void 0 ? _a : '';
